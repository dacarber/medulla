...
namespace utilities_pi0ana
{
    static std::vector<double> reco_pi0_shower_thresholds = {0.025, 0.025};
	...
	else
	{
		...
		for(size_t j(0); j < obj.particles.size(); ++j)
		{
		  ...
		  showers_metric.push_back(std::make_pair(std::make_pair(i, j), _mass));
		}
	    }
