...
namespace mctruth
{
     ...
     */
    template<typename T>
    bool single_electron(const T & obj, std::vector<double> params={70.0,})
    {
        int num_elecs(0);
    ...
    /**
     * @brief Cut for zero true final state protonss above threshold.
     * @details Applied at the GENIE generator level using obj.prim. The
     * kinetic energy is computed from the GENIE genE field.
     * @tparam T the type of the object to apply the cut on.
     * @param obj the SRTrueInteraction to apply the cut on.
     ...
     */
    template<typename T>
    bool no_protons(const T & obj, std::vector<double> params={40.0,})
    {
        int count(0);
        for(const auto & p : obj.prim)
        {
